...
public class Instrument {
  private static final String __name__ = "@Instrument ";
  ...
  public static boolean execute(String path, ASTVisitor visitor) {
    ...
    if (file.isDirectory()) {
      ...
      LevelLogger.error(
          __name__
              + "#execute input file is neither a file nor directory : "
              + file.getAbsolutePath());
      return false;
    }

    for (File f : fileList) {
      CompilationUnit unit = JavaFile.genASTFromFileWithType(f);
