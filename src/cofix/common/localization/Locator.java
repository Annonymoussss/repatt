...
public class Locator extends GZoltar {
  ...
  public Locator(String wD, Algorithm alg) throws IOException {
    super(wD);
  ...
  public List<Statement> getSuspiciousStatements() {
    ...
    for (Component c : allComponents) {
      if (c instanceof Statement) statements.add((Statement) c);
    }
    return statements;
  }

  private Response launchAgent() {
    if (this.agent != null) return Launcher.launch(this, this.agent);
    return null;
  }
}
