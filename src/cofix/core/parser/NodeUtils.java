...
public class NodeUtils {
  ...
  public static double typeSimilarity(Type t1, Type t2) {
    ...
    if (t1.toString().equals(t2.toString())
        || (t1.isPrimitiveType() && t2.isPrimitiveType() && isWidenType(t1, t2))) {
      return 1.0;
    }
    return 0.0;
  }
  ...
  public static List<Modification> handleArguments(
      Node currNode,
      int srcID,
      TYPE nodeType,
      List<Expr> srcArg,
    ...
    if (srcArg.size() == tarArgs.size()) {
      ...
      for (int i = 0; i < srcArg.size(); i++) {
        ...
        Map<SName, Pair<String, String>> tmMap =
            tryReplaceAllVariables(tExpr, varTrans, allUsableVariables);
        if (tmMap != null) {
  ...
  private static boolean canReplaceArg(Expr expr) {
    return expr instanceof SName || expr instanceof QName || expr instanceof FieldAcc;
  }

  public static boolean isSameNodeType(Node src, Node tar) {
    if (isConstant(src)) {
      return isConstant(tar);
    } else {
      return !isConstant(tar);
    }
  }
  ...
  private static boolean isConstant(Node tar) {
    ...
    if (tar instanceof QName) {
      ...
      return Character.isUpperCase(qName.getLabel().charAt(0))
          && Character.isUpperCase(qName.getIdentifier().charAt(0));
    } else if (tar instanceof SName) {
      SName sName = (SName) tar;
      return sName.getName().toUpperCase().equals(sName.getName()) || sName.getName().equals("NaN");
    }
    return false;
  }

  public static boolean conditionalMatch(
      Expr node,
      int id,
    ...
    if (node.match(first, varTrans, allUsableVariables, tmp)) {
      ...
      Map<SName, Pair<String, String>> record =
          NodeUtils.tryReplaceAllVariables(condition, varTrans, allUsableVariables);
      if (record != null) {
        NodeUtils.replaceVariable(record);
  ...
  public static boolean nodeMatchList(
      Node node,
      List<? extends Node> tarList,
      Map<String, String> varTrans,
      Map<String, Type> allUsableVariables,
      List<Modification> modifications) {
    boolean match = false;
  ...
  public static List<Modification> listNodeMatching(
      Node currNode,
      Node.TYPE nodeType,
      List<? extends Node> srcNodeList,
      List<? extends Node> tarNodeList,
      Map<String, String> varTrans,
      Map<String, Type> allUsableVariables) {
    List<Modification> modifications = new ArrayList<>();
      ...
      for (int i = 0; i < otherLen; i++) {
        if (record[i] == -1) {
          ...
          if (index != -1) {
            ...
            if (insert instanceof ReturnStmt
                || insert instanceof ThrowStmt
                || insert instanceof BreakStmt
                || insert instanceof ContinueStmt) {
              if (index != srcNodeList.size() - 1) {
                continue;
              }
            }
            if (insert instanceof WhileStmt
                || insert instanceof ForStmt
                || insert instanceof DoStmt
                || insert instanceof VarDeclarationStmt) {
              continue;
            }
            int last = index;
            for (; last >= 0; last--) {
              ...
              if (!(node instanceof ReturnStmt)
                ...
                List<SVariable> bVariables = node.getVariables();
                List<SVariable> sVariables = insert.getVariables();
                boolean dependency = false;
                for (SVariable variable : sVariables) {
  ...
  public static Map<SName, Pair<String, String>> tryReplaceAllVariables(
      Node replaceNode, Map<String, String> varTrans, Map<String, Type> allUsableVariables) {
    Map<SName, Pair<String, String>> record = new HashMap<>();
    Set<SVariable> variables = new HashSet<>(replaceNode.getVariables());
        ...
        if (!matched) {
          if (!allUsableVariables.containsKey(name)) {
            ...
            if (expr != null && !(expr instanceof NumLiteral) && !isDependencyExpr) {
              ...
              Map<SName, Pair<String, String>> tryReplace =
                  tryReplaceAllVariables(expr, varTrans, allUsableVariables);
                ...
                if (expr instanceof SName) {
                  for (Entry<SName, Pair<String, String>> entry : tryReplace.entrySet()) {
                    for (Entry<SName, Pair<String, String>> exist : record.entrySet()) {
                      if (duplicate
                          || (!exist.getKey().getName().equals(name)
                          && exist
                          .getValue()
                          .getSecond()
                          .equals(entry.getValue().getSecond()))) {
                        duplicate = true;
                        break;
                      }
                    }
            ...
            if (!canUse) {
              ...
              for (Entry<String, Type> entry : allUsableVariables.entrySet()) {
                ...
                if (type.toString().equals(typeStr)) {
                  if (!varTrans.containsValue(entry.getKey())) {
  ...
  public static boolean replaceExpr(
      int srcID,
      String avoid,
      Expr srcExpr,
      Expr tarExpr,
        ...
        if (srcMethod.getExpression() != null && tarMethod.getExpression() != null) {
          ...
          if ((src instanceof SName || tar instanceof SName)
            ...
            return false;
              ...
              .getExpression()
              .match(tarMethod.getExpression(), varTrans, allUsableVariables, tmp)) {
            return false;
          }
        }
      } else {
    ...
    if (srcExpr instanceof InfixExpr) {
      if (!(tarExpr instanceof InfixExpr)) {
        return false;
      } else if (!canReplaceOperator(
          ((InfixExpr) srcExpr).getOperator(), ((InfixExpr) tarExpr).getOperator())) {
        return false;
      }
    } else if (tarExpr instanceof InfixExpr) {
      InfixExpr infixExpr = (InfixExpr) tarExpr;
      if (infixExpr.getLhs() instanceof NumLiteral
          && NodeUtils.isBoundaryValue((NumLiteral) infixExpr.getLhs())) {
        return false;
      } else if (infixExpr.getRhs() instanceof NumLiteral
          && NodeUtils.isBoundaryValue((NumLiteral) infixExpr.getRhs())) {
        return false;
      }
    }
    ...
    if (srcType.toString().equals(tarType.toString())
        || srcType instanceof WildcardType
        || tarType instanceof WildcardType) {
      Map<SName, Pair<String, String>> record =
          tryReplaceAllVariables(tarExpr, varTrans, allUsableVariables);
      if (record != null) {
        // replace all variable
        ...
        if (!srcExpr.toSrcString().toString().equals(target) && !avoid.equals(target)) {
          Revision revision =
              new Revision(srcExpr.getParent(), srcID, target, srcExpr.getNodeType());
          modifications.add(revision);
        }
        // restore all variable
  ...
  public static boolean compatibleType(Type type1, Type type2) {
    ...
    if (isPrimitiveType(type1)) {
      if (isPrimitiveType(type2)) {
        return type1.toString().equals(type2Str)
            || isWidenType(type1, type2)
            || isWidenType(type2, type1);
      }
      return type2Str.equals("Serializable") || type2Str.startsWith("Comparable<");
    } else {
      if (isPrimitiveType(type2)) {
        return false;
      }
  ...
  public static boolean isWidenType(Type ty1, Type ty2) {
    ...
    return union != null && union.toString().equals(ty2.toString());
  }
    ...
    if (parent != null) {
      while (parent != null) {
        if (parent instanceof CompilationUnit) {
          String packageName =
              ((CompilationUnit) parent).getPackage().getName().getFullyQualifiedName();
  ...
  public static List<ASTNode> getAllSiblingNodes(ASTNode node) {
    ...
    // node.getParent().getStructuralProperty(structuralPropertyDescriptor);
    //			siblings.add(child);
    //		}
    return siblings;
  }
  ...
  public static Map<String, Type> getMutateVarTypes(String file, int line) {
    CompilationUnit unit = JavaFile.genASTFromFileWithType(file);
    MutateVariableVisitor variableVisitor = new MutateVariableVisitor(line, unit);
    unit.accept(variableVisitor);
    return variableVisitor.getVars();
  }
...
class VariableVisitor extends ASTVisitor {
  private final Map<String, Type> _vars = new HashMap<>();
  private int _line = 0;
  ...
  class MethodVisitor extends ASTVisitor {
      ...
      if (parent != null) {
        ...
        for (Object o : node.fragments()) {
          ...
          Pair<String, Type> pair =
              new Pair<String, Type>(vdf.getName().getFullyQualifiedName(), node.getType());
    ...
    public boolean visit(SingleVariableDeclaration node) {
      ...
      if (parent != null) {
        ...
        int end = _unit.getLineNumber(parent.getStartPosition() + parent.getLength());
        Pair<String, Type> pair =
            new Pair<String, Type>(node.getName().getFullyQualifiedName(), node.getType());
        Pair<Integer, Integer> range = new Pair<Integer, Integer>(start, end);
        _tmpVars.put(pair, range);
      }
      return true;
...
class MutateVariableVisitor extends ASTVisitor {
  private final Map<String, Type> _vars = new HashMap<>();
  private int _line = 0;
  private final CompilationUnit _unit;

  public MutateVariableVisitor(int line, CompilationUnit unit) {
    _line = line;
    _unit = unit;
  }
