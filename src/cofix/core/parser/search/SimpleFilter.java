...
public class SimpleFilter {
  ...
  private Set<SVariable> _variables = null;
  ...
  private final int DELTA_LINE = 10;
  ...
  public List<Pair<CodeBlock, Double>> filter(String srcPath, double guard) {
    ...
    for (Pair<CodeBlock, Double> pair : filtered) {
      ...
      double similarity =
          CodeBlockMatcher.getRewardSimilarity(_buggyCode, pair.getFirst()) + pair.getSecond();
      pair.setSecond(similarity);
    }

    Collections.sort(
        filtered,
        new Comparator<Pair<CodeBlock, Double>>() {
          @Override
  ...
  private List<Pair<CodeBlock, Double>> filter(
      List<Pair<CodeBlock, Double>> filtered, double guard) {
    //		List<Pair<CodeBlock, Double>> filtered = new ArrayList<>();
    ...
    for (CodeBlock block : _candidates) {
      ...
      //			if (codeRec.contains(block.toSrcString().toString()) ||
      // _buggyCode.hasIntersection(block)) {
    ...
    public boolean visit(SimpleName node) {
      ...
      Type type =
          ProjectInfo.getVariableType(
              classAndMethodName.getFirst(), classAndMethodName.getSecond(), name);
      SVariable variable = new SVariable(null, name, type);
      boolean match = false;
      ...
      if (match) {
        ...
        if (parent != null && !(parent.getParent() instanceof AnonymousClassDeclaration)) {
          ...
          CodeSearch codeSearch =
              new CodeSearch(_unit, line, _buggyCode.getCurrentLine(), statement);
