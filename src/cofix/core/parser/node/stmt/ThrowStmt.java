...
public class ThrowStmt extends Stmt {
  ...
  private final List<String> _insertBefore = new ArrayList<>();
    ...
    if (node instanceof ThrowStmt) {
      ...
      if (!source.equals(other._expression.toSrcString().toString())) {
        Map<SName, Pair<String, String>> record =
            NodeUtils.tryReplaceAllVariables(other._expression, varTrans, allUsableVariables);
        if (record != null) {
      ...
      if (node.getParent() != null) {
        switch (node.getParent().getUseType(node)) {
          case USE_IF:
            ...
            if (parent != null) {
              Map<SName, Pair<String, String>> record =
                  NodeUtils.tryReplaceAllVariables(parent, varTrans, allUsableVariables);
              if (record != null) {
                NodeUtils.replaceVariable(record);
                Insertion insertion =
                    new Insertion(this, INSID, parent.toSrcString().toString(), _nodeType);
