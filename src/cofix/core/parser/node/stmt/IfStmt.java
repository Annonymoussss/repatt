...
public class IfStmt extends Stmt {
  ...
  private final Expr _condition_replace = null;
  private final Expr _then_replace = null;
  private final Expr _else_replace = null;
  ...
  public boolean match(
    ...
    if (node instanceof IfStmt) {
      ...
      if (expr != null
          && _condition.toSrcString().toString().contains(expr.toSrcString().toString())) {
        match = true;
        if (_then instanceof Blk) {
          List<Node> nodes = _then.getChildren();
          modifications.addAll(
              NodeUtils.listNodeMatching(
                  _then, _nodeType, nodes, other.getSiblings(), varTrans, allUsableVariables));
        }
      } else {
  ...
  public List<SVariable> getVariables() {
    List<SVariable> list = _condition.getVariables();
    if (_then != null) {
