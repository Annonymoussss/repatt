...
public class ForStmt extends Stmt {
  ...
  private final Stmt _body_replace = null;

  /**
   * for ( [ ForInit ]; [ Expression ] ; [ ForUpdate ] ) Statement ForInit: Expression { ,
   * Expression } ForUpdate: Expression { , Expression }
   */
      ...
      if (_initializers != null && other._initializers != null) {
        List<Modification> tmp =
            NodeUtils.listNodeMatching(
                this, _nodeType, _initializers, other._initializers, varTrans, allUsableVariables);
        if (tmp != null) {
