...
public class SwitchStmt extends Stmt {
  ...
  private final int EXPID = 1000;

  /**
   * SwitchStatement: switch ( Expression ) { { SwitchCase | Statement } } SwitchCase: case
   * Expression : default :
   */
    ...
    if (node instanceof SwitchStmt) {
      ...
      modifications.addAll(
          NodeUtils.listNodeMatching(
              this, _nodeType, _statements, other._statements, varTrans, allUsableVariables));
    } else {
  ...
  public List<SVariable> getVariables() {
    List<SVariable> list = _expression.getVariables();
    for (Stmt stmt : _statements) {
