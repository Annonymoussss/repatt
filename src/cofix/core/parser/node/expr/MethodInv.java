...
public class MethodInv extends Expr {
  ...
  private static final Set<String> _avoid = new HashSet<>();
  ...
  /**
   * MethodInvocation: [ Expression . ] [ < Type { , Type } > ] Identifier ( [ Expression { ,
   * Expression } ] )
   */
    ...
    if (node instanceof MethodInv) {
      ...
      if (getType().toString().equals(other.getType().toString())) {
        ...
        if (_arguments.size() == other._arguments.size()) {
          //  OLDO : types of arguments should be compatible
          boolean compatible = true;
          for (int i = 0; i < _arguments.size(); i++) {
            if (!NodeUtils.compatibleType(
                _arguments.get(i).getType(), other._arguments.get(i).getType())) {
              compatible = false;
              break;
            }
          }
          if (compatible
              ...
              && !MethodInv._avoid.contains(_name)
              && !MethodInv._avoid.contains(other._name)
              && _arguments.size() > 0) {
      ...
      if (node instanceof ConditionalExpr) {
        ...
        if (NodeUtils.conditionalMatch(
            this, WHOLE, conditionalExpr, varTrans, allUsableVariables, tmp)) {
          match = true;
          modifications.addAll(tmp);
        }
      } else {
  ...
  public void computeFeatureVector() {
    ...
    if (_expression == null
        || !NodeUtils.skipMethodCall(_expression.toSrcString().toString(), _name)) {
