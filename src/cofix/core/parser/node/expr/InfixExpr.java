...
public class InfixExpr extends Expr {
  ...
  public boolean match(
    ...
    if (node instanceof InfixExpr) {
      List<SVariable> tarVars = node.getVariables();
      List<SVariable> srcVars = getVariables();
      for (SVariable var : tarVars) {
        String name = var.getName();
        String mapName = varTrans.get(name) == null ? name : varTrans.get(name);
        for (SVariable variable : srcVars) {
      ...
      boolean compatible =
          NodeUtils.canReplaceOperator(_operator.toString(), other._operator.toString());
        ...
        if (!other._lhs.toSrcString().toString().equals(_rhs.toSrcString().toString())) {
          if (other._lhs instanceof NumLiteral) {
            if (_lhs instanceof NumLiteral && !NodeUtils.isBoundaryValue((NumLiteral) _lhs)) {
              if (NodeUtils.replaceExpr(
                  LHSID,
                  _rhs.toSrcString().toString(),
                  _lhs,
                  other._lhs,
                  varTrans,
                  allUsableVariables,
      ...
      if ((matchLeft || matchRight)
          && compatible
          && !_operator.toString().equals(other._operator.toString())) {
      ...
      if (tarString != null
          && other.getLhs().toSrcString().toString().length() > 1
          && other.getRhs().toSrcString().toString().length() > 1) {
  ...
  public List<Pair<Expr, InfixExpression.Operator>> splitSingleBooleanExpr() {
    ...
    if (op.equals("&&") || op.equals("||")) {
      if (_lhs instanceof InfixExpr) {
        List<Pair<Expr, InfixExpression.Operator>> pair =
            ((InfixExpr) _lhs).splitSingleBooleanExpr();
      ...
      if (_rhs instanceof InfixExpr) {
        List<Pair<Expr, InfixExpression.Operator>> pair =
            ((InfixExpr) _rhs).splitSingleBooleanExpr();
        subExprs.addAll(pair);
      } else {
