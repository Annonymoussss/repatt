...
public class Assign extends Expr {
  ...
  public boolean match(
    ...
    if (node instanceof Assign) {
      ...
      List<SVariable> tarVars = assign._lhs.getVariables();
      List<SVariable> srcVars = _lhs.getVariables();
      if (tarVars.size() > 0 && srcVars.size() > 0) {
        ...
        if (source != null && source.equals(srcVars.get(0).toString())) {
          ...
          if (tarVars.get(0).getName().equals(srcVars.get(0).getName())
              && tarVars.get(0).getType().toString().equals(srcVars.get(0).getType().toString())) {
            match = true;
            List<Modification> tmp = new ArrayList<>();
            if (_rhs.match(assign._rhs, varTrans, allUsableVariables, tmp)) {
              modifications.addAll(tmp);
