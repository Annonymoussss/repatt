...
public class SName extends Label {
      ...
      if (_name.equals(other.getName())) {
        ...
        if (_exprType.toString().equals(((SName) node).getType())) {
          ...
          if (!other.getName().equals(_name)
              && allUsableVariables.containsKey(_name)
              && !NodeUtils.isClass(_name)
              && !NodeUtils.isClass(other._name)) {
            Revision revision = new Revision(this, NAMEID, other.getName(), _nodeType);
            modifications.add(revision);
          }
        } else {
          if ((_exprType.isPrimitiveType()
                  && other.getType().isPrimitiveType()
                  && NodeUtils.isWidenType(_exprType, other.getType()))
              || (_exprType instanceof WildcardType || other.getType() instanceof WildcardType)
              || _exprType.toString().equals(other.getType().toString())) {
            match = true;
            if (!other.getName().equals(_name)
                && allUsableVariables.containsKey(_name)
                && !NodeUtils.isClass(_name)
      ...
      if (node instanceof MethodInv) {
        ...
        if (_exprType.toString().equals(methodInv.getType().toString())) {
          Map<SName, Pair<String, String>> record =
              NodeUtils.tryReplaceAllVariables(methodInv, varTrans, allUsableVariables);
          if (record != null) {
            NodeUtils.replaceVariable(record);
            Revision revision =
                new Revision(this, NAMEID, methodInv.toSrcString().toString(), _nodeType);
            modifications.add(revision);
          }
        }
      }
  ...
  public List<SVariable> getVariables() {
    ...
    if (!Character.isUpperCase(_name.charAt(0))) {
      SVariable variable = new SVariable(this, _name, _exprType);
      list.add(variable);
    }
    return list;
  }
