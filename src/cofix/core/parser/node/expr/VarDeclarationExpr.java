...
public class VarDeclarationExpr extends Expr {
  ...
  private final String _declType_replace = null;
  private final String _vdfs_replace = null;

  private final int TYPEID = 0;
  private final int VDFID = 1;

  /**
   * VariableDeclarationExpression: { ExtendedModifier } Type VariableDeclarationFragment { ,
  ...
  public boolean match(
    ...
    if (node instanceof VarDeclarationExpr) {
      ...
      //  OLDO : all referenced variable should be changed correspondingly
      if (_declType.isPrimitiveType()
          && other._declType.isPrimitiveType()
          && NodeUtils.isWidenType(_declType, other._declType)) {
        match = true;
      ...
      if (match) {
        List<Modification> tmp =
            NodeUtils.listNodeMatching(
                this, _nodeType, _vdfs, other._vdfs, varTrans, allUsableVariables);
        if (tmp != null) {
