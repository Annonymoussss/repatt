...
public class CodeBlockMatcher {
  ...
  public static double getRewardSimilarity(CodeBlock buggyCode, CodeBlock codeBlock) {
    List<SVariable> buggyVars = buggyCode.getVariables();
    List<SVariable> simVars = codeBlock.getVariables();
    int total = buggyVars.size() + simVars.size();
    double varReward = 0;
    if (total != 0) {
      double varSame = 0;
      for (SVariable variable : buggyVars) {
  ...
  public static List<Modification> match(
      CodeBlock buggyBlock, CodeBlock similarBlock, Map<String, Type> allUsableVariables) {
    ...
    if (match.size() > 0) {
      for (int j = 0; j < sNodes.size(); j++) {
        if (!reverseMatch.containsKey(j)) {
          ...
          if (tarNode instanceof ReturnStmt
              || tarNode instanceof ThrowStmt
              || tarNode instanceof BreakStmt
              || tarNode instanceof ContinueStmt
              || tarNode instanceof WhileStmt
              || tarNode instanceof ForStmt
              || tarNode instanceof DoStmt
              || tarNode instanceof VarDeclarationStmt) {
            continue;
          }
          Map<SName, Pair<String, String>> record =
              NodeUtils.tryReplaceAllVariables(tarNode, varTrans, allUsableVariables);
          if (record == null) {
            continue;
          }
          ...
          if (nextMatchIndex == -1) {
            ...
            for (; last >= 0; last--) {
              ...
              if (!(node instanceof ReturnStmt)
                ...
                List<SVariable> sVariables = tarNode.getVariables();
  ...
  private static Map<String, String> matchVariables(CodeBlock buggyBlock, CodeBlock similarBlock) {
    ...
    List<SVariable> bVars = buggyBlock.getVariables();
    List<SVariable> sVars = similarBlock.getVariables();

    Map<SVariable, List<USE_TYPE>> bMap = new HashMap<>();
    for (SVariable variable : bVars) {
    ...
    Map<SVariable, List<USE_TYPE>> sMap = new HashMap<>();
    for (SVariable variable : sVars) {
    ...
    int i = 0;
    for (SVariable variable : bMap.keySet()) {
    ...
    int j = 0;
    for (SVariable variable : sMap.keySet()) {
    ...
    for (Entry<SVariable, List<USE_TYPE>> sim : sMap.entrySet()) {
      String simName = sim.getKey().getName();
      for (Entry<SVariable, List<USE_TYPE>> buggy : bMap.entrySet()) {
        String buggyName = buggy.getKey().getName();
        Double similarity =
            0.7 * LCS(sim.getValue(), buggy.getValue())
                + 0.1 * NodeUtils.nameSimilarity(simName, buggyName)
                + 0.2 * NodeUtils.typeSimilarity(sim.getKey().getType(), buggy.getKey().getType());
        if (similarity > 0.5) {
  ...
  private static Map<String, String> tryMatch(
      double[][] similarityTable,
      Map<Integer, String> reverseSimNameMap,
      Map<Integer, String> reverseBuggyNameMap) {
