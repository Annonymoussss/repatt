      ...
      for (int i = 0; i < tar.size(); i++) {
        for (; start < source.size(); ) {
          if (tar.get(i).equals(source.get(start))) {
            map.put(
                _arguments.get(start).toSrcString().toString(),
                other._arguments.get(i).toSrcString().toString());
            start++;
            break;
          }
          start++;
      ...
      for (int i = 0; i < source.size(); i++) {
        for (; start < tar.size(); ) {
          if (source.get(i).equals(tar.get(start))) {
            map.put(
                _arguments.get(i).toSrcString().toString(),
                other._arguments.get(start).toSrcString().toString());
            start++;
            break;
          }
          start++;
